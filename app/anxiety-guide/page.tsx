...
const cleanJsonResponse = (response: string): string => {
  // Remove markdown code block markers, extra whitespace, and newlines
  let cleaned = response.replace(/```json\s*|\s*```|\n/g, '').trim()
  // Remove any leading/trailing spaces
  cleaned = cleaned.replace(/^\s+|\s+$/g, '')
  console.log('Cleaned response:', cleaned) // Debug: Log cleaned response
  return cleaned
}
...
export default function RelaxingSoundsPage() {
  ...
  const getAiSounds = async () => {
    ...
    const prompt = `Based on the user's input: "${userMood}". Suggest a curated list of 2-4 relaxing sound categories and tracks to help with relaxation, focus, or sleep. Return a JSON array of categories, each with a "section" (string) and "tracks" (array of objects). Each track must have "title" (string), "description" (string), and "youtube" (valid YouTube URL with a unique video ID). Use different YouTube videos for each track, ensuring they match the input theme (e.g., ocean waves for "ocean waves"). Do not include any text outside the JSON array. Example:
[
  {
    ...
    console.log('API response for validation:', response) // Debug: Log response before parsing
    try {
      const parsed = JSON.parse(response)
      console.log('Parsed API response:', parsed) // Debug: Log parsed response
      if (Array.isArray(parsed) && parsed.length > 0 && parsed.every(category => {
        const isValidCategory = category.section && typeof category.section === 'string' &&
          Array.isArray(category.tracks) && category.tracks.length > 0 &&
          category.tracks.every((track: any) => {
            const isValidTrack = track.title && typeof track.title === 'string' &&
              track.description && typeof track.description === 'string' &&
              track.youtube && typeof track.youtube === 'string';
            console.log(`Track validation for ${track.title}:`, {
              title: track.title,
              isValidTrack,
              videoId: getYouTubeVideoId(track.youtube)
            }); // Debug: Log track validation
            return isValidTrack;
          });
        console.log(`Category validation for ${category.section}:`, { isValidCategory }); // Debug: Log category validation
        return isValidCategory;
      })) {
        setSounds(parsed)
        setAiRecommendation('AI-curated sounds loaded successfully.')
        console.log('Updated sounds state:', parsed) // Debug: Log final sounds state
      } else {
